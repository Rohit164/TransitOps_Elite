...
import { VehicleStatus, DriverStatus, TripStatus, ExpenseType } from '@prisma/client';
...
export const getDashboardKPIs = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const inMaintenance = await prisma.vehicle.count({ where: { status: VehicleStatus.IN_SHOP } });

    // 4. Retired Vehicles
    const retiredVehicles = await prisma.vehicle.count({ where: { status: VehicleStatus.RETIRED } });
    ...
    const driversOnDuty = await prisma.driver.count({ where: { status: DriverStatus.ON_TRIP } });

    // 9. Drivers Available
    const driversAvailable = await prisma.driver.count({ where: { status: DriverStatus.AVAILABLE } });
...
export const getReports = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    // 1. Fleet Utilization
    const activeVehicles = await prisma.vehicle.count({ where: { status: VehicleStatus.ON_TRIP } });
    const totalVehicles = await prisma.vehicle.count({ where: { NOT: { status: VehicleStatus.RETIRED } } });
...
export const exportCSV = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const reportType = req.query.type as string; // 'vehicles' or 'trips' or 'expenses'
